...
```{r}
#| label: tbl-salinity-classes
#| tbl-cap: "Indeling rivier en getijdenwateren gebaseerd op saliniteit."
#| warning: false
#| message: false
#| echo: false
...
salinity_table <- 
  tibble(
    `Zonering (invloed alleen NaCI)` = c(
      'Rivier',
      'Zoetwatergetijdezone', 
      'Oligohaliene zone', 
      'Mesohaliene zone', 
      'Polyhaliene zone', 
      'Euhaliene zone', 
...
knitr::kable(salinity_table)
```

De getijdenwateren worden in dit rapport onderverdeeld naar zoet, zwak brak en brak. Zoet water heeft als bovengrens 0,3 g CM. Zwak brak ligt in de range van 0,3 tot 3 en brak van 3 tot 10 g CM. Voor kanalen is het onderscheid tussen de verschillende brakke zones niet gemaakt. De indeling is gebaseerd op enerzijds de ecologische verschillen tussen gebieden en anderzijds het werkbaar houden van het stelsel in verband met het aantal ecotopen (@tbl-salinity-classes).

## Mechanische dynamiek {#sec-mechanische-dynamiek}

Mechanische dynamiek omvat alle krachten die worden uitgeoefend op zowel bodem, water, vegetatie als fauna van een ecotoop. Het gaat daarbij om erosie- en sedimentatieprocessen, transport van sediment (grind, zand en klei) en circulatiestromingen. Erosie- en sedimentatieprocessen staan niettemin centraal bij de mechanische dynamiek. In @tbl-watersysteem-dynamiek staan de factoren die de mate van dynamiek in verschillende hoofdwatersystemen van de Nederlandse rijkswateren bepalen, zoals die ook staan aangegeven in het RES, BES, MES en KES. De mechanische dynamiek wordt onderscheiden in vier verschillende klassen, waarbij de grenzen zijn gebaseerd op de kansen voor vestiging van vegetatie en/of bodemfauna (@tbl-dynamiek-klassen).

```{r}
#| label: tbl-watersysteem-dynamiek
#| tbl-cap: "Globale vergelijking indeling voor mechanische dynamiek voor verschillende watersystemen."
#| warning: false
#| message: false
#| echo: false

watersysteemfactoren <- 
  tibble(
    Hoofdwatersysteem = c(
      'Rivieren', 'Getijdenwateren', 'Meren', 'Kanalen'
    ),
    `Factoren die mechanische dynamiek bepalen` = c(
      'wisselende afvoeren van water en sediment in de rivier (rivierregime). golfwerking door scheepvaart en daarnaast ook in mindere mate wind',
'de dagelijkse richting van wisselende getijdenstromingen en de door de rivieren veroorzaakte onregelmatig wisselende afvoeren van water en sediment en daarnaast ook golfwerking door wind en scheepvaart',
'wind, strijklengte, grootte van het meer, wisseling in waterstanden, waterdiepte, stroming, type substraat of bodem en de wisselende aan- en afvoer van water en sediment',
'vaarbeweging van schepen en daarnaast ook de aan- en afvoer van water, stuw- en spuiregimes, lozingen en wind'
    )
  )

knitr::kable(watersysteemfactoren)
```


```{r}
#| label: tbl-dynamiek-klassen
#| tbl-cap: "Afbakening van de verschillende klassen voor de factor dynamiek. op basis van biologische factoren."
#| warning: false
#| message: false
#| echo: false
...
dynamiekklassen <- 
  tibble(
    Klassen = c(
      'Zeer sterk dynamisch (z)', 'Sterk dynamisch (s)', 'Dynamisch (d)', 'Laag dynamisch (I)'
    ),
    `Factoren die mechanische dynamiek bepalen` = c(
      'geen sessiele soorten: vestiging van vegetatie wordt verhinderd; bodemfauna beperkt tot enkele beweeglljke soorten; bodem is meer dan decimeters diep continu in beweging; regelmatig nieuwe standplaatsen gevormd',
      'enkel aangepaste sessiele soorten; weinig wormen. relatief veel kreeftachtigen; bodem is centimeters tot decimeters diep regel- matig in beweging',
      'relatief hoge biomassa en diversiteit aan soorten; veel variatie aan substraat; er worden nieuwe standplaatsen gevormd; bodem In beweging maar vestiging of het voorkomen van vegetatie en/of bodemfauna wordt met blijvend verhinderd',
      'voorkomen van vegetatie en/of bodemfauna wordt niet of nauwe- lijks befnvloed; relatief veel wormen'
    )
  )

knitr::kable(dynamiekklassen)
```
...
waterdiepteklassen <- 
  tibble(
    Klassen = c(
      'Zeer diep (z)', 'Diep (d)', 'Matig diep (m)', 'Ondiep (o)'
    ),
    `Begrenzende biologische factor` = c(
      'langdurige stratificatie als gevolg van verschillen in temperatuur of zout-gehalte',
      'geen langdurige stratificatie en geen uitgebreide bedekking mogelijk met waterplanten',
      'uitgebreide bedekking met waterplanten mogelijk, doch niet met helofyten', 
      'helofyten kunnen voorkomen'
    )
  )

knitr::kable(waterdiepteklassen)
```
